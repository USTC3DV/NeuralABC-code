...
if __name__ == "__main__":
    ...
    parser.add_argument('--type', type=str, default="id")
    parser.add_argument('--colli', type=bool, default=False)
    ...
    if t == "shape":
        beta1 = torch.tensor([-2.71335335, 0.13721677, -0.96557967, 2.15569137, -2.20862524,  2.02085724, 1.32704986, -0.18951267, -2.37554491,  0.31818445]).reshape(1, 10).float().cuda()
        beta2 = torch.from_numpy(np.zeros(10)).reshape(1, 10).float().cuda()
        body_code = model.get_body_code(body_num=1)
        cloth_code = model.get_cloth_code(cloth_num=5) #5
        ...
        inter = 10
        for percent in range(inter+1):
            # body_code = body_code2*(1/inter*percent)+body_code1*(1-1/inter*percent)
            beta = beta1*(1/inter*percent)+beta2*(1-1/inter*percent)
            deform_body_v, deform_body_path = model.deform_body(beta, body_v, body_t, "{}_{}".format(t, percent))
            deform_cloth_v, deform_cloth_path = model.deform_cloth(beta, cloth_v, cloth_t, "{}_{}".format(t, percent))
            if colli:
                model.deform_cloth_collision(deform_body_path, deform_cloth_path, "{}_{}".format(t, percent))


    elif t == "cloth":
        cloth_code1 = model.get_cloth_code(cloth_num=2)
        cloth_code2 = model.get_cloth_code(cloth_num=5)
        inter = 10
        for percent in range(inter+1):
            cloth_code = cloth_code1*(1/inter*percent)+cloth_code2*(1-1/inter*percent)
            cloth_v, cloth_t, cloth_path = model.reconstruct_cloth(cloth_code, "{}_{}".format(t, percent))
    

    elif t == "id":
        inter = 10
        body_code1 = model.get_body_code(body_num=0)
        body_code2 = model.get_body_code(body_num=1)
        for percent in range(inter+1):
            body_code = body_code2*(1/inter*percent)+body_code1*(1-1/inter*percent)
            body_v, body_t, body_path = model.reconstruct_body(body_code, "{}_{}".format(t, percent))

    elif t == "pose":
        body_code = model.get_body_code(body_num=1)
        cloth_code = model.get_cloth_code(cloth_num=2) #5
        body_v, body_t, body_path = model.reconstruct_body(body_code, "c")
        cloth_v, cloth_t, cloth_path = model.reconstruct_cloth(cloth_code, "c")
        pose_list = [100,101,102,103,104,105,106,107,108,109]
        beta = torch.from_numpy(np.zeros(10)).reshape(1, 10).float().cuda()
        for i in pose_list:
            pose = poses[i].reshape(1,72).float().cuda()
            cloth_path = model.cloth_lbs(cloth_v, beta, pose, cloth_t, "{}_{}".format(t, i))
            body_path = model.body_lbs(body_v, beta, pose, body_t, "{}_{}".format(t, i))
            if colli:
                model.deform_cloth_collision(t, cloth_path, "{}_{}".format(t, i))
